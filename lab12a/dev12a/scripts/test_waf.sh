...
#   4. The SAME bad request through the UNPROTECTED
#      HTTP API door                                  -> reaches the app
#
# Check 4 is the one that matters. A 403 alone only proves something
# refused you. Showing the identical payload getting through the
# other front door proves it was the WAF, not the application.
#
...
HTTP_URL="$(tf_out http_invoke_url)"
...
[[ -n "$HTTP_URL" ]] || die "no http_invoke_url output"

mkdir -p "$EVIDENCE_DIR"
TS="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
log "protected  : $REST_URL"
log "unprotected: $HTTP_URL"
log "timestamp  : $TS"
...
echo
log "=== UNPROTECTED door (HTTP API, same Lambda) ==="
C_BYPASS="$(hit unprotected_xss "$HTTP_URL" "$XSS" | tail -1)"
...
if [[ "$C_BYPASS" != "403" ]]; then
  note "OK  " "same payload reached the app via the unprotected door ($C_BYPASS) — the 403 came from the WAF, not the application"
else
  note "??  " "unprotected door also returned 403 — unexpected; investigate before claiming WAF attribution"
fi
